...
from model_pytorch import SupervisedContrastiveLoss, CrossSourceModelGRLv9
import time
...
model = CrossSourceModelGRLv9(input_channel_first=first_data.shape[1], input_channel_second=second_data.shape[1],  num_classes=n_classes, f_encoder=first_enc, s_encoder=second_enc)
...
for epoch in range(EPOCHS):
    ...
    #    x_batch_s, y_batch_s = next(iteration_dataloader_train_s)
        ...
        emb_shared_info = nn.functional.normalize( torch.cat([f_shared_discr, s_shared_discr]) )
        emb_shared_uninfo = nn.functional.normalize( torch.cat([f_shared_useless, s_shared_useless]) )

        loss_ortho = torch.mean( torch.sum(emb_dom_info * emb_dom_uninfo, dim=1) ) \
                     + torch.mean( torch.sum(emb_dom_info * emb_shared_info, dim=1) ) \
                     + torch.mean( torch.sum(emb_shared_uninfo * emb_shared_info, dim=1) ) \
