...
from model_pytorch import MonoSourceModel
...
kd_loss_name = sys.argv[4]
withNorm = int(sys.argv[5])
...
data = np.load("%s/%s_data_normalized.npy"%(dir_, second_prefix))
...
model = MonoSourceModel(input_channel_first=data.shape[1], encoder=hashPREFIX2SOURCE[second_prefix], num_classes=n_classes)
