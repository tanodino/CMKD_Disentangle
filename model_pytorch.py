...
class CrossSourceModelGRLv3(torch.nn.Module):
    ...
    def forward(self, x, lambda_val=1.):
        ...
        return f_emb_inv[:,nfeat::], f_emb_spec[:,nfeat::], s_emb_inv[:,nfeat::], s_emb_spec[:,nfeat::], self.task_dom(f_emb_spec[:,nfeat::]), self.task_dom(s_emb_spec[:,nfeat::]), self.task_cl(f_emb_inv), self.task_cl2(s_emb_inv), self.discr(grad_reverse(f_emb_inv[:,0:nfeat],lambda_val)), self.discr(grad_reverse(s_emb_inv[:,0:nfeat],lambda_val))
