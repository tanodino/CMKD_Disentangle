...
from model_pytorch import CrossSourceModelGRLv4
import time
...
model = CrossSourceModelGRLv4(input_channel_first=first_data.shape[1], input_channel_second=second_data.shape[1],  num_classes=n_classes, f_encoder=first_enc, s_encoder=second_enc)
...
for epoch in range(EPOCHS):
    ...
    #    x_batch_s, y_batch_s = next(iteration_dataloader_train_s)
        ...
        #####
        shared_discr = torch.cat([f_shared_discr, s_shared_discr])
        shared_discr = nn.functional.normalize(shared_discr)
        y_scl_sel = torch.cat([y_batch_f, y_batch_s])
        loss_contra1 = supConLoss(shared_discr, y_scl_sel)
        #####
        emb_aggr = torch.cat([f_shared_discr, f_domain_discr])     
        emb_aggr = nn.functional.normalize(emb_aggr)
        y_cl = torch.cat([y_batch_f, y_batch_f+n_classes])
        loss_contra2 = supConLoss(emb_aggr, y_cl)
        #####
        emb_aggr = torch.cat([s_shared_discr, s_domain_discr])     
        emb_aggr = nn.functional.normalize(emb_aggr)
        y_cl = torch.cat([y_batch_s, y_batch_s+n_classes])
        loss_contra3 = supConLoss(emb_aggr, y_cl)
        #####
        emb_aggr = torch.cat([f_domain_discr, f_domain_useless])     
        emb_aggr = nn.functional.normalize(emb_aggr)
        y_cl = torch.cat([y_batch_f, torch.ones_like(y_batch_f)+n_classes])
        loss_contra4 = supConLoss(emb_aggr, y_cl)
        #####
        emb_aggr = torch.cat([s_domain_discr, s_domain_useless])     
        emb_aggr = nn.functional.normalize(emb_aggr)
        y_cl = torch.cat([y_batch_s, torch.ones_like(y_batch_s)+n_classes])
        loss_contra5 = supConLoss(emb_aggr, y_cl)


        loss = loss_pred + loss_pred_dom + loss_contra1 + loss_contra2 + loss_contra3 + loss_contra4 + loss_contra5 #+ loss_contra_sel#+ loss_contra_sel#+ loss_adv_dann + loss_adv_dann #+ # + loss_contra_cl #+ loss_adv_dann#
