...
from model_pytorch import SupervisedContrastiveLoss, CrossSourceModelGRLv10
import time
...
dir_name_s = dir_+"/OUR-ABLA%d_%s"%(abla_num, second_prefix)
...
model = CrossSourceModelGRLv10(input_channel_first=first_data.shape[1], input_channel_second=second_data.shape[1],  num_classes=n_classes, f_encoder=first_enc, s_encoder=second_enc)
...
for epoch in range(EPOCHS):
    ...
    #    x_batch_s, y_batch_s = next(iteration_dataloader_train_s)
        ...
        if abla_num == 1:
            loss = loss + loss_pred_shared + loss_pred_domain_discr + loss_adv_dann  + loss_ortho
        if abla_num == 2:
            loss = loss + loss_pred_shared + loss_pred_domain_discr + loss_pred_dom  + loss_ortho
        if abla_num == 3:
            loss = loss + loss_pred_dom  + loss_adv_dann  + loss_ortho
        if abla_num == 4:
            loss = loss + loss_pred_shared + loss_pred_domain_discr + loss_pred_dom  + loss_adv_dann
